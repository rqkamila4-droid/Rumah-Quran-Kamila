document.addEventListener("DOMContentLoaded", async () => {
    ...
    async function fetchKelasBinaan() {
        try {
            const data = [
                { id: 1, nama_kelas: "Said Bin Zaid", jadwal_hari: "Sen, Rab", jam_mulai: "16:00", jam_selesai: "17:30" }
            ];
    ...
    async function fetchSantriBinaan() {
        try {
            ...
            const data = [
                { nis: "202601", nama: "Ahmad Hanif", kelas_id: 1, gender: "L" },
                { nis: "202602", nama: "Fatimah Az-Zahra", kelas_id: 1, gender: "P" }
            ];
    ...
    async function fetchDapodikTersedia() {
        try {
            dapodikListContainer.innerHTML = '<div style="text-align: center; padding: 20px;"><i class="fas fa-spinner fa-spin fa-2x" style="color:var(--color-mint-dark);"></i><p style="margin-top:10px; font-size:0.85rem; color:var(--text-muted);">Memuat Master Data...</p></div>';
            
            const data = [
                { nis: "9001", nama: "Budi Santoso", gender: "L", nama_ayah: "Rudi", nama_ibu: "Siti", alamat: "Jl. Merdeka No 1" },
                { nis: "9002", nama: "Citra Kirana", gender: "P", nama_ayah: "Andi", nama_ibu: "Maya", alamat: "Perum Asri Blok B" },
                { nis: "9003", nama: "Dika Pratama", gender: "L", nama_ayah: "Hasan", nama_ibu: "Dewi", alamat: "Jl. Pahlawan 10" },
                { nis: "9004", nama: "Eka Putri", gender: "P", nama_ayah: "Joko", nama_ibu: "Rini", alamat: "Jl. Sudirman 4" }
            ];
    ...
    // FUNGSI RENDER DAPODIK YANG SUDAH DIPERBAIKI STRUKTURNYA
    function renderDapodikList(dataToRender) {
        dapodikListContainer.innerHTML = '';
        if(dataToRender.length === 0) {
            dapodikListContainer.innerHTML = '<div style="text-align: center; color: var(--text-muted); font-size: 0.85rem; padding: 20px;">Pencarian tidak ditemukan / Semua santri sudah masuk kelas.</div>';
            return;
        }

        dataToRender.forEach(s => {
            let item = `
                <div class="dapodik-item">
                    <input type="checkbox" class="dapodik-checkbox" value="${s.nis}" id="chk_${s.nis}">
                    <div class="dapodik-info">
                        <label for="chk_${s.nis}" style="display: block; cursor: pointer; margin: 0;">
                            <div style="font-weight: 600; color: var(--text-main); font-size: 0.95rem;">${s.nama}</div>
                            <div style="color: var(--text-muted); font-size: 0.75rem; margin-top: 2px;">
                                NIS: ${s.nis} &bull; L/P: ${s.gender}
                            </div>
                            <div style="color: #95a5a6; font-size: 0.7rem; margin-top: 4px;">
                                <i class="fas fa-users" style="margin-right:4px;"></i> ${s.nama_ayah} & ${s.nama_ibu} | <i class="fas fa-map-marker-alt" style="margin-right:4px; margin-left: 4px;"></i> ${s.alamat}
                            </div>
                        </label>
                    </div>
                </div>
    ...
    if(btnSimpanKelas) {
        btnSimpanKelas.onclick = async () => {
            ...
            const stringHari = hariChecked.join(', '); // Hasil: "Senin, Rabu"
            ...
            if(hariChecked.length === 0) { alert("Pilih minimal 1 hari belajar!"); return; }
